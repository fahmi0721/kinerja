...
switch ($proses) {
	case 'DetailData':
		...
		if($JumRow > 0){
			while ($res = $query->fetch(PDO::FETCH_ASSOC)) { 
				...
				$row['Jumlah'] = rupiah($res['Jumlah'],"Rp.");
	...
	case 'Crud':
		...
		switch ($aksi) {
			case 'insert':
				try {
					$sql = "INSERT INTO tb_kelas_jabatan SET  Jumlah = :Jumlah,  KelasJabatan = :KelasJabatan, TglCreate = :TglCreate,  Keterangan = :Keterangan, UserUpdate = :UserUpdate";
			...
			case 'update':
				try {
					$sql = "UPDATE tb_kelas_jabatan SET Jumlah = :Jumlah,  KelasJabatan = :KelasJabatan, TglUpdate = :TglUpdate,  Keterangan = :Keterangan, UserUpdate = :UserUpdate WHERE IdKelasJabatan = :IdKelasJabatan";
					$query = $db->prepare($sql);
					$query->bindParam("Jumlah", $Jumlah);
					$query->bindParam("KelasJabatan", $KelasJabatan);
